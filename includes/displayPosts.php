...
if (mysqli_num_rows($postResult) > 0) {
    while ($postID = mysqli_fetch_assoc($postResult)) {
        ...
        if (!empty($postID)) {
            ...
            // Edit functionality 
            echo '<div class="container">
        <form method="post" name="editComment" action="includes/editPost.php">
        ...
        <button type="submit" class="btn btn-primary" name="postEdit">Edit</button>
        </form> 
        </div>';
            // Delete functionality
            echo '<div class="container">
        <form method="post" action="includes/deletePost.php">
        <input type="hidden" name="postID" value="' . $postID['postID'] . '">
        <button type="submit" class="btn btn-primary" name="postDelete">Delete</button>
        </form>
        </div>';
        ...
        if (mysqli_num_rows($commentResult) > 0) {
            while ($comment = mysqli_fetch_assoc($commentResult)) {
                ...
                echo '<div class="container">';
                echo '<form class="edit-form" method="post" action="../includes/editComment.php">
                    ...
                    <button type="submit" class="btn btn-primary" name="commentEdit">Edit</button>
                    </form>';

                // Delete comment functionality
                echo '<form class="delete-form" method="post" action="../includes/deleteComment.php">
                    <input type="hidden" name="commentID" class="form-control" value="' . $comment['commentID'] . '">
                    <button type="submit" class="btn btn-primary" name="commentDelete">Delete</button>
                    </form>';
                echo '</div>';
                echo '</div>';
            }
